...
``LocalJsonlRegistry`` or ``SheetsRegistry`` directly. ``registry_from_env()``
is the standard construction site — it picks the backend based on
``AUTO_AFFI__SHEETS_ID`` + service-account presence.
"""
...
def registry_from_env() -> Registry:
    ...
    service-account credential are present. Until then everything stays in
    ``data/registry/*.jsonl`` so the architecture works in dev/CI without
    cloud credentials.
    """
    ...
    if sheet_id and sa_json:
        try:
            from auto_affi.registry.sheets import SheetsRegistry
