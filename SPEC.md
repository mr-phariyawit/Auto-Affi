...
13. **Verify-Before-Spend** — before any LIVE (paid) image or video generation, the provider credit balance MUST cover the estimated job cost × safety margin, and the budget circuit-breaker (daily + per-node USD caps) MUST return ALLOW; otherwise the call is blocked. Live generation reports the real (estimated) spend, never `$0.00`. Enforced at the single adapter chokepoint (`HiggsfieldCli._pre_spend_guard`) shared by `generate_video` and `generate_image`.
