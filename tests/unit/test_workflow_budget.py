...
class TestBudgetCircuitBreaker:
    ...
    def test_default_node_caps_from_cost_model(self) -> None:
        """Default caps should match cost-model.md values."""
        breaker = BudgetCircuitBreaker()
        assert breaker.node_caps["editor_agent"] == 0.40
        assert breaker.node_caps["video_gen"] == 1.80
        assert breaker.node_caps["tts"] == 0.18
        assert breaker.node_caps["scout_strategist_llm"] == 0.05
